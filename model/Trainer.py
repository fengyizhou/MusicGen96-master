...
class MuseGANTrainer:
    ...
    def generate_inter_intra_control(self, batch_size):
        ...
        Z_16 = None
        cnt = 0
        test_z_tensor = torch.arange(-0.1, 0.1, 0.003125)
        for item in control_arr:
            ...
            # test_z_tensor = torch.arange(-0.1, 0.1, 0.003125)
            # print("test_size_is:", test_z_tensor.shape)
            # print("z_inter:", z_inter.shape)
            # for i in range(item):
            #     z_inter[i] = test_z_tensor[i]
            # if cnt == 0:
            #     Z_16 = z_inter
            # cnt+=1
            # if cnt == 2:
    ...
    def train(self, train_iter, num_epochs, model_path='', lr = 1e-4):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(train_iter):
                ...
                gpu_info = [epoch, batch_idx, -1, meminfo.used]
                gpu_used.append(gpu_info)
                if self.print_batch:
            ...
            gpu_used = np.array(gpu_used)
            np.save("gpu_test/gpu_u.npy",gpu_used)

        gp_list = np.array(gp_list)
        d_loss_list = np.array(d_loss_list)
        g_loss_list = np.array(g_loss_list)
        np.save("gp_list.npy", gp_list)
        np.save("d_loss_list.npy", d_loss_list)
        np.save("g_loss_list.npy", g_loss_list)
    ...
    def eval_sampler(self, conditions, name="gen_song"):
        ...
        print("saved:", outputs.shape)
        print(os.path.join(gen_dir, name + '.npy'))

        np.save(os.path.join(gen_dir, name + '.npy'), outputs)
        print("saved:aaaaaaaa", outputs.shape)
