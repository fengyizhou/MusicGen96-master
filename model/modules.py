...
class BarEncoder_ChordSeq(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(16, 16, (6, 1), stride=(6, 1)),  ###########2,1
            nn.BatchNorm2d(16),
            nn.LeakyReLU(0.2, True)
        )
        self.layer3 = nn.Sequential(
    ...
    def forward(self, condition):
        # condition shape: (?, 1, 48, 84) 96 85
        h0 = self.layer0(condition) # (?, 16, 48, 7) 96 17
        h1 = self.layer1(h0)  # (?, 16, 48, 1) 96 1
...
class BarGenerator_ChromaSeq(nn.Module):
    def __init__(self, z_dim):
        ...
        self.layer6 = nn.Sequential(
            nn.ConvTranspose2d(144, 64, (2, 1), stride=(2, 1), padding=0),
            nn.BatchNorm2d(64),
            nn.ReLU(True),
        )
...
class BarGenerator_ChordSeq(nn.Module):
    def __init__(self, z_dim):
        ...
        self.layer4 = nn.Sequential(
            nn.ConvTranspose2d(272, 128, (6, 1), stride=(6, 1), padding=0),  #########2,1
            nn.BatchNorm2d(128),
            nn.ReLU(True),
        )
    ...
    def forward(self, z, barCondi):
        ...
        h5 = concat_prev(h4, barCondi[1])  # (?, 144, 96, 1)
        h5 = self.layer5(h5)  # (?, 64, 48, 17)

        h6 = concat_prev(h5, barCondi[0])  # (?, 80, 96, 15)
        h6 = self.layer6(h6)  # (?, 1, 96, 85)
...
class BarDiscriminator_ChromaSeq(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(129, 128, (2, 1), stride=(2, 1)),
            nn.LeakyReLU(0.2, True)
        )
        self.layer2 = nn.Sequential(
...
class BarDiscriminator_ChordSeq(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(6, 128, (1, 12), stride=(1, 12)),  # (?, 128, 48, 7)  96 17
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 128, (1, 7), stride=(1, 7)),  # (?, 128, 48, 1)  96 1
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 128, (6, 1), stride=(6, 1)),  # (?, 128, 24, 1)  16
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 128, (2, 1), stride=(2, 1)),  # (?, 128, 12, 1)  8
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 256, (2, 1), stride=(2, 1)),  # (?, 256, 5, 1)   4
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(256, 512, (2, 1), stride=(2, 1)),  # (?, 512, 2, 1)   2
            nn.LeakyReLU(0.2, True),
        )
    ...
    def forward(self, barData, barCondi):
        # barData shape (?, 5, 48, 84)  85


        h0 = concat_prev(barData, barCondi[6])  # (?, 6, 48, 84)  85
