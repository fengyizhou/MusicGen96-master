...
def to_image_np(bars):
    ...
    ##recolored_bars = np.matmul(bars.reshape(-1, 2), colormap).reshape((bars.shape[0], bars.shape[1], bars.shape[2], 3))
...
def save_midis(bars, file_path):
    ...
    padded_bars = np.concatenate((np.zeros((bars.shape[0], bars.shape[1], 24, bars.shape[3])), bars,
                                 ...
                                 axis=2)  # made the note range from 84 to 128
    ...
    ##write_midi.write_piano_rolls_to_midi(images_with_pause_list, program_nums=[0,1], is_drum=[False, False],  \
    ##                                                        filename=file_path, tempo=80.0)

    write_midi.write_piano_rolls_to_midi(images_with_pause_list, program_nums=[0, 1, 2, 3, 4], \
...
def bislerp(a, b, c, d, steps):
    ...
    for n in range(8):
        ...
        for idx in range(1, steps + 1):
            t = step_deg * idx
            tmp = np.sin((1.0 - t) * ac_omega) / ac_so * ab_step_list[n] + np.sin(t * ac_omega) / ac_so * cd_step_list[
                n]
